...
async def collect_rss(request: RSSCollectRequest):
    ...
    if request.collect_all:
        ...
        if not feed_urls:
            return {"error": "RSS URLが設定されていません"}
        ...
        return {"error": "--feed-url または --collect-all を指定してください"}
...
async def collect_news(request: NewsCollectRequest):
    ...
    if request.collect_all:
        ...
        if not site_urls:
            return {"error": "ニュースサイトURLが設定されていません"}
        ...
        return {"error": "--site-url または --collect-all を指定してください"}
