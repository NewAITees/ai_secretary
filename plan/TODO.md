...
- AI秘書は **`src/bash_executor/`** を経由してコマンドを呼び出し、標準出力/標準エラー出力を解析する
...
- **スケジューラ・バックグラウンドジョブは別プロセス/スレッドで実行可能**
  - 例: `lifelog-system` のデーモンや定期削除ジョブは裏側で動作OK
  - これらは **Webリクエストとは独立** しており、同期を待つ必要がない
- **WSL内部の情報取得（ブラウザ履歴読み込み等）も別プロセスで実行OK**
  - Web APIをブロックしないため

### 3. プロンプト管理

- **System Promptは3ステージに分離して外部ファイル化**
  - `config/prompts/system_planner.txt`: 計画ステージ（タスク分解・リソース見積もり）
  - `config/prompts/system_executor.txt`: 実行ステージ（ツール呼び出し・結果解析）
  - `config/prompts/system_reviewer.txt`: レビューステージ（結果評価・次アクション提案）
- LLM秘書は会話フェーズに応じて適切なプロンプトを選択・結合して使用
- 編集しやすいよう、各プロンプトファイルは平文テキストで管理
